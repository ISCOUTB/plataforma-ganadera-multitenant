...
export class SaludService {
  ...
  findAllPaginated(
    ...
    const { page, limit, sortBy, sortOrder, tipo_intervencion, fincaId } =
      filters;
    // Cuando NO se pide filtro por finca conservamos el path optimizado
    // de baseRepo (sin JOIN). Sólo cuando llega `fincaId` recurrimos al
    // QueryBuilder porque la relación es indirecta (salud → bovino → finca).
    if (fincaId) {
  ...
  private async findAllPaginatedByFinca(
    ...
    const qb = this.saludRepository
      .createQueryBuilder('s')
      .innerJoin('s.animal', 'b')
      .where('s.tenant_id = :tenantId', { tenantId })
      .andWhere('s.deleted_at IS NULL')
      .andWhere('b."fincaPkIdFinca" = :fincaId', { fincaId: filters.fincaId });
    if (filters.tipo_intervencion) {
      qb.andWhere('s.tipo_intervencion = :tipo', {
        tipo: filters.tipo_intervencion,
      });
    }
    if (filters.sortBy) {
      qb.orderBy(`s.${filters.sortBy}`, filters.sortOrder ?? 'ASC');
    }
  ...
  /**
   * Transforma un DTO con `animalId` plano en un payload que TypeORM
   * mapea correctamente a la relación @ManyToOne con Animal.
   */
  private toEntityPayload(dto: { animalId?: number } & Record<string, any>): any {
    const { animalId, ...rest } = dto;
    const payload: any = { ...rest };
    if (animalId !== undefined) {
      payload.animal = animalId ? { id: animalId } : null;
    }
    return payload;
  }
