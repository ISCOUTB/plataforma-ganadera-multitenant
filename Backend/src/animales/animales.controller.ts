...
export class AnimalesController {
  ...
  @ApiOperation({ summary: 'Timeline cronológico de eventos del animal (salud, movimientos, finanzas)' })
  @ApiParam({ name: 'id', description: 'ID numérico del bovino', type: Number })
  @ApiResponse({ status: 200, description: 'Array de eventos ordenados por fecha DESC' })
  @Get(':id/timeline')
  getTimeline(
    @Param('id', ParseIntPipe) id: number,
    @Tenant() tenantId: string,
  ) {
    return this.animalesService.getTimeline(id, tenantId);
  }
