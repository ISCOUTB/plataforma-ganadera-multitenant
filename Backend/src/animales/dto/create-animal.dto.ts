...
import { EtapaProductiva } from '../entities/animal.entity';
...
export class CreateAnimalDto {
  ...
  /// Etapa productiva del bovino (producción/crecimiento/seca).
  /// Si no se envía, el service la infiere automáticamente a partir de
  /// `genero` + `fecha_nacimiento` — ver `AnimalesService.inferEtapa`.
  @ApiPropertyOptional({
    enum: EtapaProductiva,
    description:
      'Etapa productiva. Si se omite, el backend la calcula automáticamente.',
  })
  @IsOptional()
  @IsEnum(EtapaProductiva)
