...
export class FinanzasService {
  ...
  findAllPaginated(
    ...
    const { page, limit, sortBy, sortOrder, tipo_movimiento, categoria, fincaId } = filters;
   ...
   */
  async getResumen(tenantId: string, fincaId?: string): Promise<{
    total_ingresos: number;
    total_gastos: number;
    balance: number;
    cantidad_movimientos: number;
  }> {
    const qb = this.finanzaRepository
